...
public class SqlMapConfigParser extends BaseParser {
		...
		parser.addNodelet("/sqlMapConfig/typeHandler", new Nodelet() {
			public void process(Node node) throws Exception {
				...
				try {
					...
					vars.errorCtx.setMoreInfo("Check the javaType attribute '" + javaType + "' (must be a classname) or the jdbcType '" + jdbcType
							+ "' (must be a JDBC type name).");
	...
	private boolean dealSqlMapParserByInputStream(String locationPattern) throws Exception {
		if (locationPattern == null)
			return false;
		InputStream inputStream = null;
		Resource[] resources = resourcePatternResolver.getResources(locationPattern);
		for (int i = 0; i < resources.length; i++) {
			vars.errorCtx.setResource(StringUtils.substringAfterLast(resources[i].getFile().getAbsolutePath(), "classes"));
	...
	private boolean dealSqlMapParserByReader(String locationPattern) throws Exception {
		if (locationPattern == null)
			return false;
		Reader reader = null;
		Resource[] resources = resourcePatternResolver.getResources(locationPattern);
		for (int i = 0; i < resources.length; i++) {
			vars.errorCtx.setResource(StringUtils.substringAfterLast(resources[i].getFile().getAbsolutePath(), "classes"));
			reader = new InputStreamReader(resources[i].getInputStream());
			if (vars.sqlMapConv != null) {
	...
	protected void addSqlMapNodelets() {
		parser.addNodelet("/sqlMapConfig/sqlMap", new Nodelet() {
			public void process(Node node) throws Exception {
				...
				if (usingStreams) {
					isSuccessed = dealSqlMapParserByInputStream(resource) || dealSqlMapParserByInputStream(url);
				} else {
					isSuccessed = dealSqlMapParserByReader(resource) || dealSqlMapParserByReader(url);
				}
	...
	// });
	// }
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~结束~~~~~~~~~~修改iBatis对定义sqlMap的xml的解析方法使得支持*(wildcard)~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
